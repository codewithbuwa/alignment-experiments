...
def parse_args():
    parser = argparse.ArgumentParser(description="Run KTO experiment (balanced)")
...
def main():
    ...
    cfg.kto_good_fraction = 0.5
    set_seed(cfg.seed)

    timestamp = get_timestamp()
    output_root = args.output or os.path.join("results", "dpo_kto_1d", f"kto_balanced_{timestamp}")
    ...
    update_latest_paths("kto_balanced", output_root)

    print(f"Saved KTO balanced results to: {output_root}")
